package me.danbeneventano.advent2016.day7
...
fun main(args: Array<String>) {
    println(input.readLines().filter(::supportsTLS).count()) // part 1
    println(input.readLines().filter(::supportsSSL).count()) // part 2
}
...
fun supportsSSL(ip: String): Boolean {
    val abas = ip.split(Regex("(\\[|\\])")).map(::getAbas)
